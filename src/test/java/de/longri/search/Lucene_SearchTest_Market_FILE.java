...
public class Lucene_SearchTest_Market_FILE {

    static final File INDEX_FILE_HANDLE = new File("./TEST/indexer");

    @AfterAll
    static void afterAll() {
        INDEX_FILE_HANDLE.deleteOnExit();
    ...
    public void testSearchIndex() throws IOException {
        ...
        Indexer indexer = new FileSystemIndexer(INDEX_FILE_HANDLE);
        ...
        assertArrayEquals(new Integer[]{50, 38, 36, 28, 48, 41}, SEARCHER.indexes.toArray());

        SEARCHER.search("plus  nz");
        assertArrayEquals(new Integer[]{50, 38, 36, 28, 48, 41, 43, 47, 39, 40, 33}, SEARCHER.indexes.toArray());

        SEARCHER.search("plus  -nz");
        assertArrayEquals(new Integer[]{50}, SEARCHER.indexes.toArray());

        SEARCHER.search("Albanien  &nz");
        assertArrayEquals(new Integer[]{39}, SEARCHER.indexes.toArray());
    }
